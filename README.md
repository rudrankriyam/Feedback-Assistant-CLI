...
Raw inspection commands emit Apple's JSON response directly. `forms options` normalizes each question into its TAT, widget, requirement state, condition rules, and label/value choices. Draft updates fetch the current draft and form schema, preserve untouched answers, resolve labels such as `Foundation Models Framework` to Apple's submitted value, enforce Apple's 255-character text-field and 4096-character text-area limits, and then save the complete answer set.
...
This surface is unofficial and may break without notice. Draft creation, draft inspection, form option discovery, and answer updates have each been validated against Apple's web client. Attachment upload and submission remain excluded until their endpoint and response contracts are validated independently.
