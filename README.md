...
Raw inspection commands emit Apple's JSON response directly. `feedback view` reads the same server-backed detail envelope Apple's web client uses for a submitted report. `forms options` normalizes each question into its TAT, widget, requirement state, condition rules, and label/value choices. Draft updates fetch the current draft and form schema, preserve untouched answers, resolve labels such as `Foundation Models Framework` to Apple's submitted value, enforce Apple's 255-character text-field and 4096-character text-area limits, and then save the complete answer set.
